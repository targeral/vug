<template>
  <div>
      ...
      <div class="markdown" v-html="markdownContent"></div>
      <a @click="showMore" class="more" v-if="show">Read More</a>
  </div>
</template>
  ...
  export default {
    ...
    computed: {
      markdownContent() {
        ...
        let markdownHtml = markdown(this.content);
        return this.show ? markdownHtml.split('<!--more-->')[0] : markdownHtml;
      }
    },
    methods: {
      showMore () {
        console.log(this.moreFn)
        if (this.moreFn) {
          this.moreFn();
        } else {
          this.show = false;
